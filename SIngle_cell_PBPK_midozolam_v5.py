...
"""
MDZ multicellular liver PBPK (Magnolia MDZ64 style) with:
  - Explicit heterogeneous hepatocytes (n_het)
  - One BULK group representing remaining cells (N_bulk)
  - Heterogeneity via clustered NB (Gamma–Poisson) *count-based* multipliers (Option A):
      * latent counts mean = mu_count (fixed, e.g. 100)
      * dispersion = theta (log-uniform sampled per cluster)
      * cluster mean multiplier = mu_k (uniform sampled per cluster)
      * multipliers are positive and approximately continuous when mu_count is large

Key property (Option A):
  - mu_count controls discreteness (higher -> smoother)
  - theta controls overdispersion
  - mus controls relative cluster mean multipliers

Allocation:
  - Vmax_total is conserved across explicit+bulk via mean-1 rescaling of multipliers.
"""
...
# ==========================================
# Default parameters (Magnolia MDZ64 style)
# ==========================================
DEFAULT_PARAMS = dict(
    # Flows (L/min)
    Qli=1.45,
...
# ==========================================
# OPTION A: Count-based NB multipliers (Gamma–Poisson)
# ==========================================
...
def sample_clustered_nb_multipliers_optionA(
    ...
    """
    cluster_id: (n_cells,) ints 0..K-1
    mus:   (K,) cluster mean multipliers
    thetas:(K,) cluster dispersion (shape)
    """
...
# ==========================================
# RHS for multicell liver + bulk group
# ==========================================
...
# ==========================================
# Simulation wrapper with bulk group
# ==========================================
...
if __name__ == "__main__":
    ...
    # ----------------------------
    # h2 sweep: IC variance metric vs h2/Vmax
    # ----------------------------
