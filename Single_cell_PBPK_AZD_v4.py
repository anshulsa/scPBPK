...
"""
AZD1775 PBPK with multicellular BRAIN microdomains + BULK pools.

Explicit cells i=1..n_het:
  - IF microenvironment: AbrIF_i with VbrIF_i = VbrIF / N_total_cells
  - IC compartment:      AbrIC_i with VbrIC_i = VbrIC / N_total_cells

Bulk pools represent remaining cells:
  - AbrIF_bulk with V_IF_bulk
  - AbrIC_bulk with V_IC_bulk

Heterogeneity lives in expression-driven terms acting locally on IF_i:
  - uptake V -> IF_i (CLbrUptake_i)
  - efflux IF_i -> V via Pgp and ABCG2 (CLpgp_i, CLabcg2_i)

Barrier permeabilities are uniform (split across cells + bulk):
  - PSB:  V <-> IF
  - PSB2: IF <-> IC

------------------------------------------------------------
HETEROGENEITY SAMPLING (Option A; "full count", consistent with MDZ script):
  - Clustered multipliers (shared cluster label across uptake/pgp/abcg2)
  - Per-cluster mean multipliers: mus ~ Uniform(range)
  - Per-cluster dispersion: thetas ~ LogUniform(theta_range)
  - Latent counts mean: mu_count (fixed constant; larger -> smoother multipliers)
  - Sampling:
      lam ~ Gamma(shape=theta, scale=mu_count/theta)  => E[lam]=mu_count
      k   ~ Poisson(lam)
      w   = (k / mean(k)) * mu_mult   => mean(w) ~= mu_mult
------------------------------------------------------------
"""
...
# ============================================================
# Default parameters (AZD1775_model20F)
# ============================================================
DEFAULT_PARAMS = dict(
    TSTOP=24.0,
    POINTS=800,
...
# ============================================================
# Option A: Count-based clustered NB multipliers (consistent with MDZ)
# ============================================================
...
def nb_multiplier_from_counts(n, mu_mult, theta, mu_count=100.0, rng=None, floor=1e-12):
    """
    Option A:
      lam ~ Gamma(shape=theta, scale=mu_count/theta) so E[lam]=mu_count
      k   ~ Poisson(lam)
      w   = (k / mean(k)) * mu_mult

    mu_count controls discreteness (bigger = smoother).
    theta controls overdispersion (bigger = less variance).
    """
    rng = np.random.default_rng() if rng is None else rng
...
def sample_clustered_triplet_multipliers_optionA(
    ...
    """
    Shared cluster_id across uptake/pgp/abcg2.
    """
...
def alloc_total_to_cells_and_bulk(total_CL, multipliers, n_het, N_tot, name="CL"):
    """
    per-cell avg = total/N_tot
    multipliers applied to explicit cells only, then rescaled to mean 1.
    bulk gets remainder so total is conserved.
    """
...
# ============================================================
# Main (Option A; full count multipliers)
# ============================================================
if __name__ == "__main__":
    ...
    # Option A: shared mu_count (match MDZ)
    MU_COUNT = 100.0
    ...
    print("\nCluster summary (multipliers) [AZD Option A]:")
    for k in range(n_clusters):
        idx = np.where(cl == k)[0]
